...
def load_dataframe(path, lat_bnds=[31.25, 34], lon_bnds=[62, 65]):
    ds = xr.open_mfdataset(path, decode_cf=True, decode_times=True)
...
def load_all_df(directory):
    ...
    for i in result:
        print(i)
        if (
            i == "C:/CS577 Project/Data/FutureData/vas_day_UKESM1-0-LL_ssp126_r8i1p1f2_gn_20150101-20491230.nc"
        ):
            curr_df = load_dataframe(i, lat_bnds=[31.875, 34])
        elif (
            i == "C:/CS577 Project/Data/FutureData/uas_day_UKESM1-0-LL_ssp370_r8i1p1f2_gn_20150101-20491230.nc"
        ):
...
def merge_global_data(global_data):
    ...
    #     list_of_dfs,
    # )
    df_final = reduce(lambda left, right: pd.merge(left, right, on=["time", "lat", "lon"]),list_of_dfs,)
...
def main():
    ...
    path = r"C:\CS577 Project\Data\FutureData"

    save_path = r"C:\CS577 Project\Data\FutureDataSaves"

    #dataHolder = load_all_df(path)
    #pickle_dump(dataHolder, r"C:\CS577 Project\Data\FutureDataSaves\dataHolder.pkl")
    dataHolder = pickle_load(r"C:\CS577 Project\Data\FutureDataSaves\dataHolder.pkl")
    ...
    # need to preserve actual lat and lon values, but create matching lat and lon values for merge
    print(filtered_data)
    filtered_data[-1] = filtered_data[-1].rename(columns={"lat": "vas_lat"})
    filtered_data[-2] = filtered_data[-2].rename(columns={"lon": "uas_lon"})
    filtered_data[-4] = filtered_data[-4].rename(columns={"lat": "rsdsdiff_lat", "lon": "rsdsdiff_lon"})
    ...
    clf = pickle_load(
        r"C:\CS577 Project\Data\FutureDataSaves\random_forest.pkl"
    )
    scaler = pickle_load(
         r"C:\CS577 Project\Data\FutureDataSaves\minmaxclf.pkl"
    )

    wet_reg_model = pickle_load(
         r"C:\CS577 Project\Data\FutureDataSaves\wet_random_forest_regression.pkl"
    )
    dry_reg_model = pickle_load(
         r"C:\CS577 Project\Data\FutureDataSaves\dry_random_forest_regression.pkl"
    )

    wet_scaler = pickle_load(
         r"C:\CS577 Project\Data\FutureDataSaves\minmax_scaler_regression_wet.pkl"
    )
    dry_scaler = pickle_load(
         r"C:\CS577 Project\Data\FutureDataSaves\minmax_scaler_regression_dry.pkl"
    )
    ...
    # save result in csv
    result_df.to_csv(
        r"C:\CS577 Project\Data\FutureDataSaves\predicted_precipitation_2023-2034.csv"
    )
